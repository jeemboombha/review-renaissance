...
const DashboardCard = ({ icon, title, count, onClick, highlight = false }) => {
  return (
    <div 
      onClick={onClick}
      className={`relative h-[180px] p-6 rounded-xl ${highlight ? 'glass border-primary/20' : 'glass'} flex flex-col justify-between cursor-pointer transition-all duration-200 hover:shadow-md hover:translate-y-[-2px]`}
    >
      {highlight && (
